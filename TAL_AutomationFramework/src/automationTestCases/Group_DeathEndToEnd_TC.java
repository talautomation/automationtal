...
public class Group_DeathEndToEnd_TC {
	...
	@Test
	public void GroupDeath_EndToEnd() throws Exception {
		...
		// ClaimIntake_page.tbl_PersonSearchResults(cmanagerdriver)
		// Selecting_RightElement.Select(cmanagerdriver, sGroupId, allPersons,
		// 3);
		...
		if ((InsuredTitle.getFirstSelectedOption().getText()).equals("Unknown")) {
			InsuredTitle.selectByVisibleText("Dr");
		...
		if (ClaimIntake_page.txt_DOBDay(chiefdriver).getText().equals("")) {
			...
			ClaimIntake_page.txt_DOBDay(chiefdriver).sendKeys("01");
			Select DOBMonth = new Select(ClaimIntake_page.sel_DOBMonth(chiefdriver));
			DOBMonth.selectByVisibleText("JUN");
			ClaimIntake_page.txt_DOBYear(chiefdriver).sendKeys("1955");
		...
		if ((prefContMet.getFirstSelectedOption().getText()).equals("Unknown")) {
			prefContMet.selectByVisibleText("Mail");
		}
		...
		Selecting_RightElement.Select(chiefdriver, "Employer", allDivisions, 1);

		List<WebElement> allClasses = ClaimIntake_page.tbl_Classes(chiefdriver)
				.findElements(By.cssSelector("tbody > tr"));
		Selecting_RightElement.Select(chiefdriver, "Death", allClasses, 0);
		...
		Thread.sleep(1000);
		// Input Medical Code
		ClaimIntake_page.txt_DiagnosisCode(chiefdriver).sendKeys("hip");
